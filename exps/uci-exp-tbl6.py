...
for dat, (n_samples, sz) in dat_size_dict.items():
    root_cmd = 'python bnn_stein_f.py -dataset={} -valid '.format(dat)
    param_specs = {
        'layers': ['50 50'],
        'seed': list(range(1334, 1354)),
        'lr': [4e-3],
        'n_particles': [20],
        ('batch_size', 'n_epoch'): [[100, 1000]],
        'ptb_scale': [1],
        'test_freq': [20]
    }
    if n_samples > 1000:
        param_specs[('batch_size', 'n_epoch')] = [[1000, 1000]]

    if dat in set(['protein_data']):
        param_specs['layers'] = ['100 100']
...
r = runner.Runner(n_max_gpus=3, n_multiplex=1, n_max_retry=1)
r.run_tasks(tasks)

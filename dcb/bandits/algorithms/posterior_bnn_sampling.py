...
from bandits.algorithms.svgd_model import SVGDModel
...
class PosteriorBNNSampling(BanditAlgorithm):
  ...
  def __init__(self, name, hparams, bnn_model='RMSProp'):
    ...
    if bnn_model == 'Variational':
      ...
      self.bnn = SVGDModel(hparams, bnn_name)
    else:
      self.bnn = NeuralBanditModel(self.optimizer_n, hparams, bnn_name)

    self.bnn_model_name = bnn_model + "-" + bnn_name
  ...
  def action(self, context):
    ...
    with self.bnn.graph.as_default():
      ...
      fd = {self.bnn.x: c}
      if isinstance(self.bnn, SVGDModel):
        fd = {
          self.bnn.x: np.tile(c, [1 + self.bnn.hparams.interp_batch_size, 1]),
          self.bnn.data_size: 10
        }
      output = self.bnn.sess.run(self.bnn.y_pred, feed_dict=fd)
      if isinstance(self.bnn, SVGDModel) and output.shape[0] == 0:

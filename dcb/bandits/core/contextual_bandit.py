...
def run_contextual_bandit(context_dim, num_actions, dataset, algos, opt_rewards):
  ...
  assert list(opt_rewards.shape) == [num_contexts]
  ...
  for i in range(num_contexts):
    ...
    if i % 100 == 0:
      print(
        'Cumulative reward:',
        ' '.join([
          "{0}: {1:.2f}".format(a.bnn_model_name, np.sum(h_rewards[:, j]))
          for j, a in enumerate(algos)]),
        'Optimal: {0:.2f}'.format(np.sum(opt_rewards[:i]))
      )

...
def BNReLU1(x, name=None):
    x = BatchNorm('bn', x, center=False, scale=False)
    return tf.nn.relu(x)
  ...
  def build_graph(self, image, label):
    ...
    def residual(name, l, increase_dim=False, first=False):
      ...
      with tf.variable_scope(name):
        b1 = l if first else BNReLU1(l)
        c1 = Conv2D('conv1', b1, out_channel, strides=stride1, activation=BNReLU1)
    ...
    with argscope([Conv2D, AvgPooling, BatchNorm, GlobalAvgPooling], data_format='channels_first'), \
      ...
      l = Conv2D('conv0', image, 16, activation=BNReLU1)
    ...
    wd_w = tf.constant(0.0002)#tf.train.exponential_decay(0.0002, get_global_step_var(),
#                     480000, 0.2, True)
...
if __name__ == '__main__':
  ...
  BATCH_SIZE = BATCH_SIZE // get_nr_gpu()
  logger.auto_set_dir()
